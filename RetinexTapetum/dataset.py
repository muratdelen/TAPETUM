"""Dataset utilities for LOL-v2 paired low/normal images.

The dataset keeps the same pairing rule across all models: a sample is valid
only if the low-light file name also exists inside the target normal-light
folder. This avoids accidental misalignment between low and ground-truth images.
"""
...
class LOLPairDataset(Dataset):
    ...
    Validation mode keeps the full image unchanged so model quality can be
    measured on the original test content.
    """
    ...
    def _resize_if_needed(self, low, high):
        """Upscale small images so random cropping is always valid."""
        w, h = low.size
    ...
    def _random_crop(self, low, high):
        """Take the same random patch from both low and target images."""
        w, h = low.size
    ...
    def _augment(self, low, high):
        """Apply simple geometric augmentation while preserving pair alignment."""
