...
parser = argparse.ArgumentParser(description='RetinexNet for LOLv2 Real_captured')
...
parser.add_argument('--test_dir', dest='test_dir', default='./datasets/LoLv2/LOL-v2/Real_captured/Test/Low', help='directory for testing inputs')
...
# =========================================================
# LOLv2 Real_captured dataset paths
# =========================================================
TRAIN_LOW_DIR  = '../datasets/LoLv2/LOL-v2/Real_captured/Train/Low'
TRAIN_HIGH_DIR = '../datasets/LoLv2/LOL-v2/Real_captured/Train/Normal'
EVAL_LOW_DIR   = '../datasets/LoLv2/LOL-v2/Real_captured/Test/Low'
...
def lowlight_train(lowlight_enhance):
    ...
    train_low_data_names = glob(os.path.join(TRAIN_LOW_DIR, '*.*'))
    train_high_data_names = glob(os.path.join(TRAIN_HIGH_DIR, '*.*'))

    train_low_data_names.sort()
    train_high_data_names.sort()

    assert len(train_low_data_names) == len(train_high_data_names), \
        "Train Low ve Train Normal dosya sayıları eşit değil."

    print('[*] Number of training data: %d' % len(train_low_data_names))
    print('[*] Train Low Dir :', TRAIN_LOW_DIR)
    print('[*] Train High Dir:', TRAIN_HIGH_DIR)
    ...
    eval_low_data = []
    eval_low_data_name = glob(os.path.join(EVAL_LOW_DIR, '*.*'))
    eval_low_data_name.sort()

    print('[*] Number of eval data: %d' % len(eval_low_data_name))
    print('[*] Eval Low Dir:', EVAL_LOW_DIR)
...
def lowlight_test(lowlight_enhance):
    ...
    print('[*] Number of test data: %d' % len(test_low_data_name))
