...
class DecomNetRetinexTapetumRGBModel(nn.Module):
    def __init__(self, base=32, lambda_init=1.0, channel_gate_scale=0.25, lambda_max=1.6):
        super().__init__()
    ...
    def forward(self, low, high=None):
        ...
        lam = torch.clamp(F.softplus(self.lambda_param), min=0.0, max=self.lambda_max)
