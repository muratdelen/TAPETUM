...
from config import (
    ...
    EPOCHS, LR, SEED, BASE_CHANNELS, LAMBDA_INIT,
    CHANNEL_GATE_SCALE, LAMBDA_MAX, PATIENCE
)
from dataset import LOLPairDataset
from model import DecomNetRetinexTapetumRGBModel
...
def train_one_epoch(model, loader, optimizer, device):
    model.train()
    running = {
        "total": 0.0, "l1": 0.0, "ssim": 0.0, "color": 0.0,
        "attn": 0.0, "gate": 0.0, "decomp": 0.0,
        "recon_low": 0.0, "recon_high": 0.0, "reflect": 0.0,
        "smooth_low": 0.0, "smooth_high": 0.0
    }
    ...
    for batch in pbar:
        ...
        gate = output["channel_gate"].detach().cpu().numpy()
...
def main():
    ...
    model = DecomNetRetinexTapetumRGBModel(
        base=BASE_CHANNELS,
    ...
    with torch.no_grad():
        init_gate = model.tapetum_net.channel_gate_scale * torch.tanh(model.tapetum_net.alpha) + 1.0
        init_gate = init_gate.detach().cpu().numpy()

    print(f"Initial channel gate -> R: {init_gate[0]:.4f}, G: {init_gate[1]:.4f}, B: {init_gate[2]:.4f}")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        with torch.no_grad():
            gate = 1.0 + model.tapetum_net.channel_gate_scale * torch.tanh(model.tapetum_net.alpha)
            gate = gate.detach().cpu().numpy()
            lam = torch.clamp(F.softplus(model.lambda_param), min=0.0, max=LAMBDA_MAX).item()
        ...
        log_str = (
            ...
            f"lambda {lam:.4f} | "
            f"gate=[{gate[0]:.4f}, {gate[1]:.4f}, {gate[2]:.4f}]"
        )
        print(log_str)
