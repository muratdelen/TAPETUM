"""
Training script for RetinexTapetumRGB.

This version is standardized to match the reference training structure as closely as
possible so you can compare outputs across models more reliably.
"""
...
from utils import seed_everything, calc_psnr, save_history_csv
...
def train_one_epoch(model, loader, optimizer, device):
    ...
    for batch in pbar:
        ...
        for k in ("total", "l1", "ssim", "color", "attn", "gate", "smooth_enh"):
            running[k] += logs[k]
        running["psnr"] += psnr
...
def main():
    ...
    with torch.no_grad():
        init_gate = 1.0 + model.tapetum_net.channel_gate_scale * torch.tanh(model.tapetum_net.alpha)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        save_history_csv(history, LOG_CSV_PATH)
    ...
    print("History CSV:", LOG_CSV_PATH)
